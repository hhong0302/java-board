...
%>
				<tr>
					...
					<td class="content-td col-md-5">
						<a href="/board/QandA?count=<%=count%>&bbstitle=<%=bbstitle%>" class="contentlink">
						<%if(ischecked==0){%><%=content%><%}else{%>비밀 글입니다.<%}%>
						</a>
					</td>
...
<%  }%>
	...
	<div class="mt-3 mb-5 row">
		<ul class="pagination justify-content-center mb-2">
			<%
				...
				if(startNum>1)
				{
					...
					out.print("<li class=\"page-item\"><a class=\"page-link\" href=\"?fname=boards/qanda&page="+prevPage+"\">이전</a></li>");
				}
				else
				{
				...
				%>
					<li class="page-item <%=act%>"><a class="page-link" href="?fname=boards/qanda&page=<%=i%>"><%=i%></a></li>
				<%
				}
				...
				if(endNum<totalPage)
				{
					...
					out.print("<li class=\"page-item\"><a class=\"page-link\" href=\"fname=boards/qanda&page="+nextPage+"\">다음</a></li>");
				}
				else
				{
		...
		<div class="d-flex justify-content-end newboard">
			<a href="?fname=boards/bwrite&bbstitle=qanda">새 글 작성</a>
		</div>
		<%} %>
	</div>
</div>
